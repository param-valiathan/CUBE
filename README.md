...
- Writes `model/session_env_context.json` (per-bin time series — including per-bin centroid `x`/`y` position,
  added for the Paradigm Results tab's occupancy heatmaps (v6 part 3), plus `dist_to_region_boundary_nose`
  and, when a boundary shape was traced, `nose_outside_boundary` — plus session summaries, including
  `region_roles`/`object_roles` and `object_interaction_bout_lengths_sec`, and paradigm-gated derived
...
- **Every sub-view** (not just Generic) gets the condensed heatmap+trajectory grid described below in place of a single pooled heatmap, a shared arena/region group-by-region and cluster-by-region cross-tab, and every bar chart in the tab overlays each individual animal's own value as an unfilled ring on top of its group's bar. Outline labels are placed with a collision-avoidance pass (`_smart_outline_label_anchors`) so a nested/annular region (e.g. Open Field's Periphery, whose naive polygon centroid lands back at the arena center on top of Center's own label) gets pushed to a point actually inside that region instead of overlapping.
- **Condensed heatmap+trajectory grid** (`build_group_heatmap_trace_figure`), one column per experimental group, pooled occupancy heatmap on top and that group's median-activity animal's raw track underneath, sharing one colorbar — replaces a single heatmap pooling every loaded animal regardless of group, in every sub-view (Generic, Y-Maze, EPM, Discrimination Index, Sociability, CPP; Approach/Avoid Events keeps its own event-trajectory-arrow plot instead, since color there already encodes approach/avoid classification).
- **Small single-purpose bar charts condensed into one narrow row per sub-view**, instead of each getting a full-width figure of its own: Generic's activity/thigmotaxis controls (region time, total distance, center-zone entries); EPM's locomotor/exploratory controls (% open-arm entries, latency to open arm, total arm entries — % time in open arms stays a full headline chart); Discrimination Index's validity/activity controls (total exploration time, latency to first contact); Three-Chamber's activity controls (chamber entries, latency to stranger — both index charts stay full-size headline metrics). Y-Maze and CPP each have only one control metric, so they were left as single full-size charts (a "row" of one saves nothing).
- **Generic (Open Field) sub-view layout**: (1) the heatmap+trajectory grid; (2) Group-by-region occupancy (the primary between-group comparison) immediately after; (3) the condensed activity-control bar row; (4) center-zone time course and mean-speed distribution; (5) Cluster-by-region occupancy last. "Save Current Data as CSV" exports every underlying table (region cross-tabs, every bar chart's own per-animal values, the representative-animal-per-group list) alongside "Save Current Graph(s)", in every sub-view — not just Generic.
- The Group-by-region and Cluster-by-region cross-tabs answer different questions and are normalized accordingly: Group-by-region is a per-group weighted-mean % of *that group's own* time spent in each region (does not sum to 100% across groups). Cluster-by-region gets **two** companion charts, both present in every sub-view: one normalized so each region's bars sum to 100% *across* clusters — "of all the time animals spent in this region, what fraction went to each cluster" (`aggregate_region_time_share_by_label`) — and one keeping the original per-cluster weighted-mean-of-*its own* time metric — "of cluster 7's own time, what fraction was in the Center?" (`aggregate_region_time_by_label`, does not sum to 100% across clusters).
- **Behavior groups, not raw clusters, once any are defined**: when the Group Editor has one or more named behavior groups, both cluster-by-region charts plot *only* those groups (by name/color) — a cluster the user left out of every group is dropped from the plot entirely, since leaving a cluster out of every group is the Group Editor's only way to exclude it (there is no separate per-cluster hide control). With no behavior groups defined at all, every cluster is shown under its raw ID and the same app-wide deterministic cluster-id→color mapping used in Unbiased Analytics/Cluster Hierarchy, since nothing has excluded it.
...
- **Time-course, ethogram, distribution, and paired-comparison plots** (added after the initial build): a %-of-trial time-course (mean ± SEM per group) for Open Field center-zone occupancy, EPM open-arm occupancy, Discrimination Index/Sociability investigation distance to the role-tagged object, and CPP cumulative chamber crossings; a Y-Maze arm-entry ethogram strip (one row per session, colored by occupied arm, hub shown in gray); investigation bout-duration distributions and latency-to-first-contact bars for Discrimination Index and Sociability; an Open Field mean-speed distribution; and, for CPP under Repeated Measures, a paired pre→post dumbbell plot (one line per matched subject). Backed by four new additive `session_env_context.json` keys (`region_roles`, `object_roles`, `dist_to_region_boundary_nose`, `nose_outside_boundary`, `object_interaction_bout_lengths_sec` — existing keys unchanged). EPM also gets a head-dip/peering-out proxy bar (mean fraction of time the nose point crossed outside a traced arena boundary) — explicitly a geometric proxy, not a validated behavioral classifier, and only shown when a boundary was traced. See [Behavioral_Paradigms_Reference.md](Behavioral_Paradigms_Reference.md) §2.3 for full detail.
...
| Step 1's `filter_dlc_h5` smoothed straight through low-confidence/missing frames, corrupting nearby high-confidence ones | `cube_core.py: filter_dlc_h5()` | Confirmed on real data: the median/Gaussian filter chain ran directly on raw x/y with no likelihood check at all, so DLC's "no detection" sentinel and near-zero-confidence detections were smoothed in like real coordinates. On one real session this dragged a `nose` keypoint through a ~410 px round-trip excursion it never made, and measurably displaced frames up to ~10 frames away that were themselves 100% confident, purely because they shared a filter window with the bad region. Root cause: this function never read the likelihood column, and (separately) `find_dlc_files()` prefers `*_filtered.h5` as the pipeline's primary input, so the corruption fed straight into Step 2's own likelihood-aware interpolation with no way to detect it — a frame reporting 100% confidence had no signal that its coordinate had already been silently altered. Fixed by making this filter jitter-removal only: confident (`>= pcutoff`, default 0.6) frames are grouped into maximal runs, a run shorter than 7 frames is left untouched (not enough same-quality neighbours to trust), and each qualifying run is filtered *in isolation* — so a run's filtered output can only ever depend on its own samples, never bleed across a gap from a neighbouring run or the low-confidence frames themselves, which now always keep their original raw value. Gap-filling remains entirely Step 2's job, unchanged. Verified against real data: max deviation on already-confident frames dropped from ~410 px to single digits (median ~2.7 px, the genuine smoothing effect) across three independent test sessions |
