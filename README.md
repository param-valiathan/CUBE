...
- **Adaptive parallel HDBSCAN sweep + system resource management (Aug 2026):** the primary `min_cluster_size`
  ...
  majority of a run's wall-clock time. It now dispatches via the same thread-pool pattern as
  `hdbscan_split_n_jobs` above (`hdbscan_sweep_n_jobs`, default `-1`), with selection unchanged (best-score pick
  over an unordered candidate list — verified byte-identical to the old sequential order on real data). New
