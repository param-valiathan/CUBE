...
conda activate CUBE
pip install pillow opencv-python-headless scipy scikit-learn umap-learn customtkinter plotly hmmlearn deeplabcut aniposelib imageio-ffmpeg
...
- Optional cluster-stability seed sweep: re-runs UMAP + HDBSCAN (+ the split/merge refinement pass, when
  ...
  Adjusted Rand Index → `plots/cluster_stability.png`. `seed_sweep_n` defaults to `6` (on by default) —
...
- `cluster_hierarchy_enabled` (default `True`) — saves `plots/cluster_hierarchy.png`, a dendrogram of the final clustering's cluster centroids in feature space (`cluster_hierarchy_linkage`: `"ward"`/`"average"`/`"complete"`), to guide manual merging decisions.
- `seed_sweep_n_jobs` (`1` = sequential default | `2`–cpu_count | `-1`) — parallelises the cluster-stability seed sweep's per-seed UMAP+HDBSCAN(+refinement) fits across worker threads (joblib, thread-based — process-based `loky` workers were tried and hit a real Windows access-violation crash from concurrent numba first-time JIT compilation, so this follows the same thread-pool pattern already used by `hdbscan_split_n_jobs`). Speeds up `seed_sweep_n > 0` runs only; no effect otherwise.
...
| Cluster Validity | Silhouette diagram + 2D embedding scatter for the *primary* HDBSCAN clustering (loaded from the run's saved `umap_embedding.npy`/labels) — complements the other seven modes, which all operate on post-hoc reclustering of aggregated per-cluster behaviour stats rather than raw pose features |
...
<output_dir>/
  ...
  plots/
    ...
    cluster_hierarchy.png                   ← (if cluster_hierarchy_enabled) feature-space dendrogram
